                    ...
                    <div id="tableHover" class="col-lg-12 col-12 layout-spacing">
                        <div class="statbox widget box box-shadow">
                            ...
                            <div class="widget-content widget-content-area" style="padding-top: 0px;">
								<form action="{{ url('/'.Request::segment(1).'/edit/'.Crypt::encrypt($promotion->id)) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
								{{ csrf_field() }}
								...
								<div class="form-group row mb-2">
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<select class="form-control basic" name="year">
												...
												@for($i=2021;$i<=date('Y');$i++)
													<option value="{{ $i }}" @if($promotion->year==$i) selected @endif>{{ $i }}</option>
												@endfor
											</select>
									...
									<div class="form-group row mb-2">
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<select class="form-control basic" name="period">
												...
												<option value="April" @if($promotion->period=="April") selected @endif>April</option>
												<option value="Oktober" @if($promotion->period=="Oktober") selected @endif>Oktober</option>
											</select>
											@if ($errors->has('year')) <div class="invalid-feedback" style="display: block;margin-top: -20px;">{{ $errors->first('year') }}</div>@endif
										</div>
									</div>
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<select class="form-control basic" name="employee_id" id="employee_id" onchange="getClass()">
												...
												@foreach($employee as $v)
													<option value="{{ $v->id }}" @if($promotion->employee_id==$v->id) selected @endif>{{ $v->nip}} || {{ $v->front_title }} {{ $v->name }} {{ $v->back_title }}</option>
												@endforeach
											</select>
									...
									<div class="form-group row mb-4">
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<input type="text" class="form-control" name="last_promotion" id="last_promotion" value="{{ $promotion->last_promotion }}" readonly/>
									...
									<div class="form-group row mb-4">
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<input type="text" class="form-control" name="new_promotion" id="new_promotion" value="{{ $promotion->new_promotion }}" readonly/>
									...
									<div class="form-group row mb-2">
										...
										<div class="col-xl-9 col-lg-9 col-sm-10">
											<select class="form-control basic" name="promotion_type">
												...
												<option value="Pejabat Negara" @if($promotion->promotion_type=="Pejabat Negara") selected @endif>Pejabat Negara</option>
												<option value="Prestasi Luar Biasa" @if($promotion->promotion_type=="Prestasi Luar Biasa") selected @endif>Prestasi Luar Biasa</option>
												<option value="Penyesuaian Ijazah" @if($promotion->promotion_type=="Penyesuaian Ijazah") selected @endif>Penyesuaian Ijazah</option>
												<option value="Jabatan Fungsional Tertentu" @if($promotion->promotion_type=="Jabatan Fungsional Tertentu") selected @endif>Jabatan Fungsional Tertentu</option>
												<option value="Jabatan Struktural" @if($promotion->promotion_type=="Jabatan Struktural") selected @endif>Jabatan Struktural</option>
												<option value="Reguler" @if($promotion->promotion_type=="Reguler") selected @endif>Reguler</option>
											</select>
