			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<input type="text" data-kt-class-table-filter="search" class="form-control form-control-solid w-300px ps-14 btn-sm " name="search" placeholder="Cari Golongan" />
					</div>
					</form>
					<!--end::Search-->
				</div>
				...
				<div class="card-toolbar">
					...
					<div class="d-flex justify-content-end" data-kt-class-table-toolbar="base">
						...
						@can('tambah-data')
							<a href="{{ url('/'.Request::segment(1).'/create') }}" class="btn btn-primary btn-sm me-2 mb-2"><i class="fa fa-plus"></i>Tambah Golongan</a>
						@endcan
			...
			<div class="card-body pt-0">
				...
				<table class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300 gy-4 gs-4" id="kt_table_classs">
					...
					<tbody>
						@foreach($class as $v)
						<tr>
							<td>{{ ($class ->currentpage()-1) * $class ->perpage() + $loop->index + 1 }}</td>
							<td>{{ $v->code }}</td>
							<td>{{ $v->class }}</td>
							<td>{{ $v->rank }}</td>
							<td>
				...
				<!-- PAGINATION -->
				<div style="div-align:right">{{ $class->appends(Request::only('search'))->links() }}</div>
				</div>
				<!--end::Table-->
			</div>
			<!--end::Card body-->
