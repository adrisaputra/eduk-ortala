...
<script>
	...
	function getEmployee(id){
		// Mendapatkan nilai ID dengan menghapus "leave_history-" dari awal ID
		var leave_historyId = id.replace("leave_history-", "");
		console.log("ID pegawai yang diklik:", leave_historyId);
		...
		url = "{{ url('/leave_history/edit') }}"
		editButton.href = ""+url+"/"+leave_historyId;
	}
	...
	document.addEventListener("click", function(event) {
		...
		if (!clickedElement.matches("td[id^='leave_history-']") && clickedElement !== editButton) {
...
function tampil(){
    ...
    url = "{{ url('/leave_history/search') }}/{{ Request::segment(2) }}"
    $.ajax({
