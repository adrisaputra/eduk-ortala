							...
							@endif
								...
								<div id="hasil">
									...
									<table class="table table-bordered table-hover mb-4">
										...
										<tbody>
										@foreach($punishment_history as $v)
											<tr onclick="selectRow(this)">
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ ($punishment_history ->currentpage()-1) * $punishment_history ->perpage() + $loop->index + 1 }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->nip }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->sk_number }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->sk_date }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->official_name }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->punishment }}</td>
												<td id="punishment_history-{{ $v->id }}" onClick="getEmployee(this.id)">{{ $v->desc }}</td>
											</tr>
										@endforeach
										</tbody>
									</table>
									<div class="paginating-container">{{ $punishment_history->appends(Request::only('search'))->links() }}</div>
									</div>
								</div>
                            </div>
                        </div>
...
<script>
	...
	function getEmployee(id){
		// Mendapatkan nilai ID dengan menghapus "punishment_history-" dari awal ID
		var punishment_historyId = id.replace("punishment_history-", "");
		console.log("ID pegawai yang diklik:", punishment_historyId);
		...
		url = "{{ url('/punishment_history/edit') }}"
		editButton.href = ""+url+"/"+punishment_historyId;
	}
	...
	document.addEventListener("click", function(event) {
		...
		if (!clickedElement.matches("td[id^='punishment_history-']") && clickedElement !== editButton) {
...
function tampil(){
    ...
    url = "{{ url('/punishment_history/search') }}"
    $.ajax({
        ...
        swal({
            ...
            if (result.value) {
                    ...
                    'success'
						...
						url = "{{ url('/punishment_history/delete') }}"
                        $.ajax({
                            url:""+url+"/"+id+"",
                            success: function(response){
                                location.reload();
