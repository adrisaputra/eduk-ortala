	...
	<table class="table table-bordered table-hover mb-4">
		...
		@foreach($presence_recapitulation as $v)
			<tr>
				...
				@if($parent_unit_id == 100)
					<td style="width:30%">{{ $v->parent_unit->name }}</td>
				@endif
				<td style="width:30%">{{ $v->day }}, {{ date('d-m-Y', strtotime($v->date)) }}</td>
				<td style="width:30%">{{ $v->employee_amount }}</td>
				<td style="width:30%">{{ $v->tl }}</td>
				<td style="width:30%">{{ $v->ct }}</td>
				<td style="width:30%">{{ $v->s }}</td>
				<td style="width:30%">{{ $v->h }}</td>
				<td style="width:30%">{{ $v->th }}</td>
				<td style="width:30%">
					@php
						$x = $v->h; 
