...
<script>
	...
	function getEmployee(id){
		// Mendapatkan nilai ID dengan menghapus "training_history-" dari awal ID
		var training_historyId = id.replace("training_history-", "");
		console.log("ID pegawai yang diklik:", training_historyId);
		...
		url = "{{ url('/training_history/edit') }}"
		editButton.href = ""+url+"/"+training_historyId;
	}
	...
	document.addEventListener("click", function(event) {
		...
		if (!clickedElement.matches("td[id^='training_history-']") && clickedElement !== editButton) {
...
function tampil(){
    ...
    url = "{{ url('/training_history/search') }}/{{ Request::segment(2) }}"
    $.ajax({
        ...
        swal({
            ...
            if (result.value) {
                    ...
                    'success'
						...
						url = "{{ url('/training_history/delete') }}"
                        $.ajax({
                            url:""+url+"/"+id+"",
                            success: function(response){
                                location.reload();
