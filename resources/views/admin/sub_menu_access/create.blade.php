	...
	<div id="kt_content_container" class="container-xxl">
		...
		<div class="card">
			...
			<div class="card-body pt-0">
					...
					<form action="{{ url('/'.Request::segment(1).'/'.Crypt::encrypt($group->id).'/'.Crypt::encrypt($menu->id)) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
					{{ csrf_field() }}
								...
								<div class="mb-10">
									...
									<select class="form-select" aria-label="Select example" name="sub_menu_id">
										...
										@foreach($sub_menu as $v)
											<option value="{{ $v->id }}" @if(old('sub_menu_id')==$v->id) selected @endif>{{ $v->sub_menu_name }}</option>
										@endforeach
									</select>
