	...
	<div id="kt_content_container" class="container-xxl">
		...
		<div class="card">
			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<input type="text" data-kt-education-table-filter="search" class="form-control form-control-solid w-300px ps-14 btn-sm " name="search" placeholder="Cari Pendidikan" />
					</div>
					</form>
					<!--end::Search-->
				</div>
				...
				<div class="card-toolbar">
					...
					<div class="d-flex justify-content-end" data-kt-education-table-toolbar="base">
						...
						@can('tambah-data')
							<a href="{{ url('/'.Request::segment(1).'/create') }}" class="btn btn-primary btn-sm me-2 mb-2"><i class="fa fa-plus"></i>Tambah Pendidikan</a>
						@endcan
			...
			<div class="card-body pt-0">
				...
				<table class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300 gy-4 gs-4" id="kt_table_educations">
					<!--begin::Table head-->
					<thead>
						<!--begin::Table row-->
					...
					<tbody>
						...
						<tr>
							<td>{{ ($education ->currentpage()-1) * $education ->perpage() + $loop->index + 1 }}</td>
							<td>{{ $v->code }}</td>
							<td>{{ $v->name }}</td>
							<td>{{ $v->level_code }}</td>
							<td>
				...
				<!-- PAGINATION -->
				<div style="div-align:right">{{ $education->appends(Request::only('search'))->links() }}</div>
				</div>
				<!--end::Table-->
			</div>
			<!--end::Card body-->
